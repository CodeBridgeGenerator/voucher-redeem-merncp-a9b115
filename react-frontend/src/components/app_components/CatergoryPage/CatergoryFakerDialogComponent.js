...
import catergoryFakerFactory from "./catergoryFakerFactory";
...
const CatergoryFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = catergoryFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("catergory").create(elem));
...
export default connect(mapState, mapDispatch)(CatergoryFakerDialogComponent);

...
import cartitemsFakerFactory from "./cartitemsFakerFactory";
...
const CartitemsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cartitemsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cartitems").create(elem));
...
export default connect(mapState, mapDispatch)(CartitemsFakerDialogComponent);

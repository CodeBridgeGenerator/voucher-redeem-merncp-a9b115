...
const CartitemsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            id: _entity?.id,voucherid: _entity?.voucherid,userid: _entity?.userid,quantity: _entity?.quantity,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Cartitems created successfully" });
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="quantity" className="w-full mb-3" value={_entity?.quantity} onChange={(e) => setValByKey("quantity", e.value)}  />
            </span>
            <small className="p-error">

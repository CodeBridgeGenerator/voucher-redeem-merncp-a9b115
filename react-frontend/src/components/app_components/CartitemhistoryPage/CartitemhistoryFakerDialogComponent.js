...
import cartitemhistoryFakerFactory from "./cartitemhistoryFakerFactory";
...
const CartitemhistoryFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cartitemhistoryFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cartitemhistory").create(elem));
...
export default connect(mapState, mapDispatch)(CartitemhistoryFakerDialogComponent);
